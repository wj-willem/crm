...
public class ClueServiceImpl implements ClueService {
    ...
    public ClueEchartsResultVo queryClueEcharts() {
        ...
        example.createCriteria().andEqualTo("typeCode", "source");
        List<DictionaryValue> dictionaryValues = dictionaryValueMapper.selectByExample(example);
        //将所有的线索来源名称存放在一个list<String>中
        List<String> sourceName = new ArrayList<>();

        for (DictionaryValue dictionaryValue : dictionaryValues) {
            sourceName.add(dictionaryValue.getValue());
        }
        //根据线索来源对线索进行分组，将查询的结果集封装在一个List<Map<String,String>>中
        List<Map<String, String>> listmap = clueMapper.queryClueEcharts();
        Map<String, List<Map<String, String>>> map = new HashMap<>();
        map.put("source", listmap);
        //将查询出来所有数据封装到对象中
        ClueEchartsResultVo chartVo = new ClueEchartsResultVo();
        chartVo.setDataMap(map);
        chartVo.setDataList(sourceName);
        return chartVo;
