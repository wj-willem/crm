...
<div id="create-transaction2"
    ...
    <form id="tranForm" method="post">
        ...
        <div class="form-group" style="width: 400px;position: relative; left: 20px;">
            <label>市场活动源&nbsp;&nbsp;<a href="javascript:void(0);" data-toggle="modal" data-target="#searchActivityModal"
...
<script>
    function convert() {
        ...
        if ($('#isCreateTransaction').prop('checked')) {
            //创建交易
            $('#tranForm').prop('action', '${pageContext.request.contextPath}/workbench/clue/convert?isCreateTransaction=1');
            $('#tranForm').submit();
        } else {
            location.href = "${pageContext.request.contextPath}/workbench/clue/convert?isCreateTransaction=0&clueId=${clue.id}";
        }
    ...
    $('#activityName').keypress(function (event) {
        ...
        if (event.keyCode == 13) {
            $.ajax({
                url: '${pageContext.request.contextPath}/workbench/clue/queryActivityIncludeNow',
                data: {
                    'clueId': '${clue.id}',
